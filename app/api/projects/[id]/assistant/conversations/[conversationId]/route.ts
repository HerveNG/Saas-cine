...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string; conversationId: string }> }) {
  ...
  const { data: conversation } = await supabase.from("ai_conversations").select("id,role,title").eq("id", conversationId).eq("project_id", id).eq("user_id", user.id).maybeSingle();
