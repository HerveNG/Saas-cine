...
export async function GET(request: Request, { params }: Params) {
  ...
  const { data: workflows, error } = await supabase
    .from("ai_workflows")
    .select("id,title,goal,status,current_task_index,created_at,updated_at,completed_at,ai_tasks(id,task_index,role,objective,output_label,status,output,action_ids,error_message,started_at,completed_at)")
...
export async function POST(request: Request, { params }: Params) {
  ...
  const { data: created, error: workflowError } = await supabase
    .from("ai_workflows")
    .insert({ project_id: projectId, user_id: user.id, title: workflow.title, goal: goal || workflow.title })
    .select("id,title,goal,status,current_task_index,created_at,updated_at")
    .single();
  ...
  const tasks = workflow.tasks.map((task, index) => ({
    ...
    user_id: user.id,
    task_index: index,
    role: task.role as AIRole,
    objective: task.objective,
    output_label: task.output,
