...
export async function POST(request: Request, { params }: Params) {
  ...
  const { data: created, error: workflowError } = await supabase.from("ai_workflows").insert({ project_id: projectId, user_id: user.id, package_id: selectedPackage?.id ?? null, title: workflow.title, goal: goal || workflow.title }).select("id,title,goal,package_id,status,current_task_index,created_at,updated_at").single();
