...
export default function AIUsagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (!projectId) return;
    fetch(`/api/projects/${projectId}/assistant/usage`)
  ...
  if (!data) return <main className="min-h-screen bg-[#090909] p-8 text-white/50">Chargement de l’usage IA…</main>;
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        <header className="border-b border-white/10 pb-6">
          ...
          <div className="mt-3 flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div><h1 className="text-3xl font-semibold tracking-tight">Usage IA</h1><p className="mt-2 text-sm text-white/45">{data.project.title} · consommation et exécution des agents</p></div>
            <a href={`/projects/${projectId}/assistant`} className="rounded-lg border border-white/10 px-4 py-2 text-xs uppercase tracking-wider text-white/70 hover:border-amber-300/40 hover:text-white">← Assistant IA</a>
          </div>
        </header>

        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {[['Exécutions', t.completed + t.failed], ['Tokens', fmtNumber(t.totalTokens)], ['Entrée', fmtNumber(t.inputTokens)], ['Sortie', fmtNumber(t.outputTokens)], ['Actions', t.actions]].map(([title, value]) => (
            <div key={title} className="rounded-xl border border-white/10 bg-white/[0.025] p-5"><p className="text-[10px] uppercase tracking-[0.22em] text-white/35">{title}</p><p className="mt-3 text-2xl font-semibold">{value}</p></div>
          ))}
        </section>

        <section className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-xl border border-white/10 bg-white/[0.025] p-6">
            <div className="flex items-center justify-between"><h2 className="text-sm font-semibold uppercase tracking-[0.18em]">Coût estimé</h2><span className="text-xs text-white/35">{data.pricing.currency}</span></div>
            <p className="mt-5 text-4xl font-semibold text-amber-200">{fmtCost(t.cost, data.pricing.currency)}</p>
            <p className="mt-2 text-xs text-white/40">Tarification configurée à partir de AI_INPUT_PRICE_PER_1M et AI_OUTPUT_PRICE_PER_1M.</p>
            <div className="mt-6 grid grid-cols-2 gap-3"><div className="rounded-lg bg-white/[0.035] p-4"><p className="text-xs text-white/40">Durée cumulée</p><p className="mt-1 font-medium">{(t.durationMs / 1000).toFixed(1)} s</p></div><div className="rounded-lg bg-white/[0.035] p-4"><p className="text-xs text-white/40">Échecs</p><p className="mt-1 font-medium">{t.failed}</p></div></div>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/[0.025] p-6"><h2 className="text-sm font-semibold uppercase tracking-[0.18em]">Par agent</h2><div className="mt-5 space-y-3">{data.byRole.map((item: any) => <div key={item.role} className="flex items-center justify-between border-b border-white/5 pb-3"><div><p className="text-sm">{label(item.role)}</p><p className="text-xs text-white/35">{item.runs} exécution(s) · {fmtNumber(item.tokens)} tokens</p></div><span className="text-xs text-amber-200/80">{fmtCost(item.cost, data.pricing.currency)}</span></div>)}</div></div>
        </section>
