...
export default function ProjectEditor({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => { params.then(({ id }) => { setId(id); fetch(`/api/projects/${id}`).then(async r => { if (r.ok) setForm(await r.json()); else setMessage("Projet introuvable."); }); }); }, [params]);
  ...
  async function submit(event: FormEvent) {
    ...
    const result = await response.json(); setLoading(false); setMessage(response.ok ? "Projet enregistré." : result.error || "Erreur d’enregistrement.");
  }
  return <main style={{ minHeight: "100vh", padding: "40px 6vw", maxWidth: 1000 }}>
    <a href={`/projects/${id}`} style={{ color: "#a1a1aa" }}>← Projet</a>
...
function Area({ label, value, change, name }: any) { return <label style={{ display: "grid", gap: 8 }}><span>{label}</span><textarea name={name} rows={7} value={value} onChange={(e) => change(name, e.target.value)} style={{ ...input, resize: "vertical" }} /></label>; }
